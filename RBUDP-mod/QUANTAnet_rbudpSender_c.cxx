...
void QUANTAnet_rbudpSender_c::send_threads( void *buf, int bufSize, int sendRate, int packetSize, int me, int nth ) {
  int start = (bufSize / nth)*me;
  char *buffer = (char*) buf;
  int size = me == nth-1 ? ((bufSize / nth)*(me+1) - start) : bufSize - start;
  QUANTAnet_rbudpSender_c *sendt = new QUANTAnet_rbudpSender_c( 38000 + 2*(me+1) );
  sendt->init( rHost );

  printf("Thread %d on the move!\n",me);
  sendt->send( buffer + start, size, sendRate, packetSize );
}
...
void QUANTAnet_rbudpSender_c::send(void * buffer, int bufSize, int sendRate, int packetSize)
{
  ...
  while (!done)
  {
    // Receive rate from receiver
    n = readn( tcpSockfd, (char*) &sendRate, sizeof( int ) );
    if( n < 0 ) {
      perror( "read send rate" );
      exit(1);
    }
    usecsPerPacket = 8 * payloadSize * 1000 / (sendRate+1);
    ...
    srate = (double) remainNumberOfPackets * payloadSize * 8 / ((double) reportTime(curTime) + 1);	
    ...
    n = readn(tcpSockfd, errorBitmap, sizeofErrorBitmap);
    if (n<0) {
      perror("read");
      exit(1);
    }
...
int QUANTAnet_rbudpSender_c::sendfile(int sendRate, int packetSize)
{
  ...
  int ncores;
  // Get number of cores from receiver
  if( readn( tcpSockfd, (char*)&ncores, sizeof( ncores ) ) < 0 ) {
    fprintf(stderr, "reading number of cores failed.\n");
    return( FAILED );
  }
...
#pragma omp parallel
  {
    send_threads( buf, filesize, sendRate, packetSize, omp_get_thread_num(), ncores );
  }
  //pthread_t *threads = (pthread_t*) malloc( ncores * sizeof( pthread_t ) );
  //struct thread_args *ta = (struct thread_args*) malloc( ncores * sizeof( struct thread_args ) );
  //for( int i = 0; i < ncores; i++ ) {
  //  ta[i].buf = buf;
  //  ta[i].bufSize = filesize;
  //  ta[i].sendRate = sendRate;
  //  ta[i].packetSize = packetSize;
  //  ta[i].me = i;
  //  ta[i].nth = ncores;
  //  pthread_create( threads + i, NULL, send_threads, NULL );
  //}
  //for( int i = 0; i < ncores; i++ )
